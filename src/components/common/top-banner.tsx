...
export default function TopBanner() {
	...
	function closeBanner() {
		bannerRef.current?.classList.remove('h-9');
		bannerRef.current?.classList.add('h-0');
	}
	return (
		<div ref={bannerRef} className="bg-black h-9 text-[11px] sm:text-sm overflow-hidden transition-all">
			<div className="flex sm:justify-center text-white relative px-4 lg:px-8 xl:px-24 max-w-[80rem] mx-auto">
				<p className="py-2">
					Sign up and get 20% off to your first order.{' '}
					<a
						href=""
						className="underline decoration-solid font-bold underline-offset-4"
					>
						Sign Up Now
					</a>
				</p>
				<svg
					...
					className="absolute h-3 w-3 sm:h-auto sm:w-auto right-4 lg:right-8 xl:right-24 top-1/2 -translate-y-1/2 cursor-pointer"
					onClick={closeBanner}
				>
					<path
